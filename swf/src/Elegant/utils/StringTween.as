...
package Elegant.utils
{
	...
	public class StringTween extends EventDispatcher
	{
		...
		private var firstMeetComma:Boolean = true;
		public var running:Boolean;
		
		public var name:String;
		public var delayComma:Boolean;
		/**
		 ...
		 */
		public function StringTween(vars:Object = null)
		{
			ticker = new Shape();
		 ...
		 * @param	delay
		 */
		public function run(startStr:String, endStr:String, duration:Number, ease:Function = null, sampleString:String = null, delay:Number = 0):void
		{
			reset( { startString:startStr, endString:endStr, duration:duration, delay:delay, sampleString:sampleString, ease:ease } );
		}
		/**
		...
		private function updateString(ratio:Number):void
		{
			...
			else
			{
				...
				if (delayComma && firstMeetComma && startString.charAt(pendingLength).search(/\W/) != -1)
				{
					firstMeetComma = false;
					ticker.removeEventListener(Event.ENTER_FRAME, tickHandler);
					startString = currentString;
					TweenNano.delayedCall(.3, play);
					return;
				}
				firstMeetComma = true;
			}
