...
{	
	...
	public class Reflection extends Sprite
	{
		...
		private function createReflection(event:Event = null):void
		{
			...
			var arrAlpha:Array = [_numStartFade, (_numStartFade - _numEndFade) / 2, _numEndFade];
			var arrMatrix:Array = [0, 0xFF * _numMidLoc, 0xFF];
			matGrad.createGradientBox(_disTarget.width, _disTarget.height, 0.5 * Math.PI);
			shpDraw.graphics.beginGradientFill(GradientType.LINEAR, [0, 0, 0], arrAlpha, arrMatrix, matGrad)
